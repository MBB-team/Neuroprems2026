function [sub_data] = taskRatingN_V2023(sub_data, cfg)
% function [sub_data] = taskRatingN_V2023(sub_data, cfg)
%
...
% text items
itemList = readstim('SocialNormViolations.xlsx');
...
while ntrial<nTraining
    ...
    while exit==0
        ...
        answer_names = {'Complètement acceptable','Complètement inacceptable'};
...
while ntrial<nTrial
    ...
    while exit==0
        ...
        % Write instructions
        question_name = 'Je trouve cela';
        answer_names = {'Complétement acceptable','Complètement inacceptable'};
        draw_scale_instruction_2(display.window,x,y,question_name,answer_names,ftsz_small);
        Screen('DrawTexture',display.window,emoji_neutral,[],rect_emoji_neutral);
        Screen('DrawTexture',display.window,emoji_infinite,[],rect_emoji_infinite);
...
% saving
sub_data.(cfg.sessNber_str).tasks.taskRatingN.results = struct('trainingData', training_data, 'data', data);
% Do we really need to save this ?
sub_data.(cfg.sessNber_str).tasks.taskRatingN.trainingList = trainingList;
sub_data.(cfg.sessNber_str).tasks.taskRatingN.itemList = itemList;
