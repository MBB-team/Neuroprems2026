...
subjects = [77:79 100:105 200:203 400:403];
sess = 1;
...
for nsub = subjects
    for nsess = sess
        ...
        if data{i,1} >=200 && data{i,1}<210
            data{i,2} = 'FTD';
            counterFTD = counterFTD+1;
        elseif (data{i,1} >=77 && data{i,1}<80) || (data{i,1} >= 100 && data{i,1} <200)
            data{i,2} = 'CTRL';
            counterCTRL = counterCTRL+1;
        elseif data{i,1} >= 400 && data{i,1} <500
            data{i,2} = 'AD';
...
i=0;
%glm_modelsR2E2 = NaN(3, length(subjects));

glm_models.WeightR2E2 = struct('AD', NaN(3, length(counterAD)), ...          % Field for WeightR2E2 task
                                'FTD', NaN(3, length(counterFTD)), ...
                                'CTRL', NaN(3, length(counterCTRL)));
glm_models.Delay_img = struct('AD', NaN(3, length(counterAD)), ...           % Field for Delay_img task
                              'FTD', NaN(3, length(counterFTD)), ...
                              'CTRL', NaN(3, length(counterCTRL)));
glm_models.Delay_txt = struct('AD', NaN(3, length(counterAD)), ...           % Field for Delay_txt task
                              'FTD', NaN(3, length(counterFTD)), ...
                              'CTRL', NaN(3, length(counterCTRL)));
glm_models.Delay_both = struct('AD', NaN(3, length(counterAD)), ...          % Field for both Delay tasks
                               'FTD', NaN(3, length(counterFTD)), ...
                               'CTRL', NaN(3, length(counterCTRL)));
glm_models.Handgrip = struct('AD', NaN(3, length(counterAD)), ...            % Field for GripRP task
                             'FTD', NaN(3, length(counterFTD)), ...
                             'CTRL', NaN(3, length(counterCTRL)));
...
for nsub = subjects
    for nsess = sess
        ...
        surf = readmatrix('/Users/adriaanapsv/desktop/MotiscanV2024/task_materials/text/greylevel_socialnorms.xlsx');
        ...
        % Extract WeightR2E2 measures

        if isfield(data{i, 5}, "taskWeightR2E2")

            data{i,28} = mean(data{i,5}.taskWeightR2E2.results.data.isAccept); %Global acceptance rate
            data{i,29} = median(data{i,5}.taskWeightR2E2.results.data.RT); % Median Reaction Time

            % Select data for the current participant
            participant_data = data{i, 5}.taskWeightR2E2.results.data;
            
            X = [participant_data.ratingBenefit, participant_data.ratingCost];  % Predictor variables
            y = participant_data.isAccept;                                      % Response variable (assuming it's binary, 0 or 1)
        
            % Fit the GLM for the current participant
            if data{i,1} >=200 && data{i,1}<210
                glm_models.WeightR2E2.FTD(:, glmFTD) = glmfit(X, y, 'binomial');
                glmFTD = glmFTD + 1;
            elseif (data{i,1} >=77 && data{i,1}<80) || (data{i,1} >= 100 && data{i,1} <200)
                glm_models.WeightR2E2.CTRL(:, glmCTRL) = glmfit(X, y, 'binomial');
                glmCTRL = glmCTRL + 1;
            elseif data{i,1} >= 400 && data{i,1} <500
                glm_models.WeightR2E2.AD(:, glmAD) = glmfit(X, y, 'binomial');
                glmAD = glmAD + 1;  % why only 2 AD data???
            end
                                                                             %glm_models.WeightR2E2.(group)(:, i) = glmfit(X, y, 'binomial');

        else
            disp(['subject',num2str(nsub),'has no data for taskWeightR2E2']);
            data{i,28} = NaN;
            data{i,29} = NaN;
        ...
        % Extract Choice DelayR2 (img) measures
        if isfield(data{i, 5}, "taskChoiceDelayR2")

            data{i,30} = median(data{i,5}.taskChoiceDelayR2.results.data.RT); % Median Reaction Time

            immediateResponsesLocation = data{i,5}.taskChoiceDelayR2.results.data.immIsLeft; 

            if immediateResponsesLocation == 1                            
                delayedResponse = 1 - data{i,5}.taskChoiceDelayR2.results.data.isLeftChoice; % 0 is delayed response, on the right
                data{i,31} = mean(delayedResponse);                         %Global acceptance rate for delayed response

                

            else                                                           
                delayedResponse =  data{i,5}.taskChoiceDelayR2.results.data.isLeftChoice; % 1 is delayed response, on the left
                data{i,31} = mean(delayedResponse);                         %Global acceptance rate for delayed response

                participant_data = data{i, 5}.taskChoiceDelayR2.results.data;


                for k = 1:numel(data{i, 5}.taskChoiceDelayR2.results.data.delayLabel)
                    text_delay_label = data{i, 5}.taskChoiceDelayR2.results.data.delayLabel(k,:);
                    if text_delay_label == "Demain"
                        data{i, 5}.taskChoiceDelayR2.results.data.delayNumbers(k,11) = 1;

                    elseif text_delay_label == "Dans trois jours"
                        data{i, 5}.taskChoiceDelayR2.results.data.delayNumbers(k,11) = 3;

                    elseif text_delay_label == "Dans une semaine"
                        data{i, 5}.taskChoiceDelayR2.results.data.delayNumbers(k,11) = 7;

                    elseif text_delay_label == "Dans deux semaines"
                        data{i, 5}.taskChoiceDelayR2.results.data.delayNumbers(k,11) = 14;

                    elseif text_delay_label == "Dans un mois"
                        data{i, 5}.taskChoiceDelayR2.results.data.delayNumbers(k,11) = 30;

                    elseif text_delay_label == "Dans trois mois"
                        data{i, 5}.taskChoiceDelayR2.results.data.delayNumbers(k,11) = 90;

                    elseif text_delay_label == "Dans six mois"
                        data{i, 5}.taskChoiceDelayR2.results.data.delayNumbers(k,11) = 180;
                    
                    elseif text_delay_label == "Dans un an"
                        data{i, 5}.taskChoiceDelayR2.results.data.delayNumbers(k,11) = 365;

                    elseif text_delay_label == "Dans trois ans"
                        data{i, 5}.taskChoiceDelayR2.results.data.delayNumbers(k,11) = 1095;
                    
                    end

                    interaction_delay_rating = participant_data.ratingRight .* data{i, 5}.taskChoiceDelayR2.results.data.delayNumbers;
                    X = [participant_data.ratingLeft, participant_data.ratingRight, data{i, 5}.taskChoiceDelayR2.results.data.delayNumbers, interaction_delay_rating];  % Predictor variables
                    y = participant_data.isLeftChoice;                                  % Response variable (assuming it's binary, 0 or 1)
                
                    % Fit the GLM for the current participant
                    if data{i,1} >=200 && data{i,1}<210
                        glm_models.Delay_img.FTD(:, glmFTD) = glmfit(X, y, 'binomial');
                        glmFTD = glmFTD + 1;
                    elseif (data{i,1} >=77 && data{i,1}<80) || (data{i,1} >= 100 && data{i,1} <200)
                        glm_models.Delay_img.CTRL(:, glmCTRL) = glmfit(X, y, 'binomial');
                        glmCTRL = glmCTRL + 1;
                    elseif data{i,1} >= 400 && data{i,1} <500
                        glm_models.Delay_img.AD(:, glmAD) = glmfit(X, y, 'binomial');
                        glmAD = glmAD + 1;  % why only 2 AD data???
                    end
            ...
            % for the glm here, the predictor variables need to be delay
            % (in days???), and rating difference between the two items or rating of delayed + immediate rewards (i.e., 3 predictors?) 


        else
            disp(['subject',num2str(nsub),'has no data for taskChoiceDelayR2']);
            data{i,30} = NaN;
            data{i,31} = NaN;
        end


            % + how to make discounting curve (+ food vs non-food)

            % + analyses for food vs non-food items
            
            % + correlation between acceptance rate and mean rating
            % difference for each different delay (i.e. 8 diforetika
            % relationships anamesa se ratings, acceptance rate + specific
            % delay)


        % Extract Choice Delay R2 (txt) measures
        if isfield(data{i, 5}, "taskChoiceDelayR2Text")

            data{i,32} = median(data{i,5}.taskChoiceDelayR2Text.results.data.RT); % Median Reaction Time

            immediateResponsesLocation = data{i,5}.taskChoiceDelayR2Text.results.data.immIsLeft; 
            if immediateResponsesLocation == 1                            
                delayedResponse = 1 - data{i,5}.taskChoiceDelayR2Text.results.data.isLeftChoice; % 0 is delayed response, on the right
                data{i,31} = mean(delayedResponse);                         %Global acceptance rate for delayed response

            else                                                           
                delayedResponse =  data{i,5}.taskChoiceDelayR2Text.results.data.isLeftChoice; % 1 is delayed response, on the left
                data{i,31} = mean(delayedResponse);                         %Global acceptance rate for delayed response
            end


        else 
            disp(['subject',num2str(nsub),'has no data for taskChoiceDelayR2Text']);
            data{i,32} = NaN;
            data{i,33} = NaN;
        end
      

        % Comparisson of both delay tasks

        
        % Extract Handgrip measures

        % Do I need to normalise data for handgrip?
...
%% plot results
% idx{1} = find(strcmp(data(:,2),'Pilot'));
idx{2} = find(strcmp(data(:,2),'AD'));
idx{3} = find(strcmp(data(:,2),'FTD'));
idx{4} = find(strcmp(data(:,2),'CTRL'));

%groupname = {'FTD','AD','CTRL'};
groupname = {'AD','FTD','CTRL'};
groupcol = {[1 0 0],[0.11 0.7 0.7],  [0.42 0.66 0.11], [0.91 0.49 0.07]};
%groupcol = {[1 0 0], [0 1 0], [0 0 1], [0.5 0.5 0.5]};
varname = {'','','','','','Reward rating','Reward rating time','Effort rating',...
...
[h,p,~,stats]=ttest2([data{idx{2},6}],[data{idx{4},6}]);
...
[h,p,~,stats]=ttest2([data{idx{2},8}],[data{idx{4},8}])
...
[h,p,~,stats]=ttest2([data{idx{2},10}],[data{idx{4},10}])
...
[h,p,~,stats]=ttest2([data{idx{2},12}],[data{idx{4},12}])
...
[h,p,~,stats]=ttest2([data{idx{3},14}],[data{idx{4},14}])

disp('ttest Choice R21D results (MA vs Ctrl)');
[h,p,~,stats]=ttest2([data{idx{2},14}],[data{idx{4},14}])
...
nanmean([data{idx{4},14}])

% Plot Choice E21D results
for nvar = 16:17%%%%%CHANGE NUMBERS
    figure('Color','w','Position',[0 0 300 300]);
    hold on;
    for ngroup = 2:length(idx)
        bar(ngroup,mean([data{idx{ngroup},nvar}]),'FaceColor',groupcol{ngroup});
...
[h,p,~,stats]=ttest2([data{idx{2},16}],[data{idx{4},16}])

% mean consistency E21D for FTD
nanmean([data{idx{3},16}])
% mean consistency E21D for MA
nanmean([data{idx{2},16}])
% mean consistency E21D for CTRL
nanmean([data{idx{4},16}])

% Plot Choice N1D results
for nvar = 18:19%%%%%CHANGE NUMBERS
    figure('Color','w','Position',[0 0 300 300]);
    hold on;
    for ngroup = 2:length(idx)
        bar(ngroup,mean([data{idx{ngroup},nvar}]),'FaceColor',groupcol{ngroup});
...
[h,p,~,stats]=ttest2([data{idx{2},18}],[data{idx{4},18}])

% mean consistency N1D for FTD
nanmean([data{idx{3},18}])
% mean consistency N1D for MA
nanmean([data{idx{2},18}])
% mean consistency N1D for CTRL
nanmean([data{idx{4},18}])

% Plot Choice Control Perception results
for nvar = 20:21%%%%%CHANGE NUMBERS
    figure('Color','w','Position',[0 0 300 300]);
    hold on;
    for ngroup = 2:length(idx)
        bar(ngroup,mean([data{idx{ngroup},nvar}]),'FaceColor',groupcol{ngroup});
...
[h,p,~,stats]=ttest2([data{idx{3},20}],[data{idx{4},20}])
disp('ttest Choice Control Perception results (MA vs Ctrl)');
[h,p,~,stats]=ttest2([data{idx{2},20}],[data{idx{4},20}])


% mean consistency perception for FTD
nanmean([data{idx{3},20}])%%%%%CHANGE NUMBERS
% mean consistency perception for MA
nanmean([data{idx{2},20}])%%%%%CHANGE NUMBERS
% mean consistency perception for CTRL
nanmean([data{idx{4},20}])%%%%%CHANGE NUMBERS


% Plot Choice Control Semantic results
for nvar = 22:23 %%%%%CHANGE NUMBERS
    figure('Color','w','Position',[0 0 300 300]);
    hold on;
    for ngroup = 2:length(idx)
        bar(ngroup,mean([data{idx{ngroup},nvar}]),'FaceColor',groupcol{ngroup});
...
disp('ttest Choice Control TOM (DFT vs Ctrl)');%%%%%CHANGE NUMBERS
[h,p,~,stats]=ttest2([data{idx{3},24}],[data{idx{4},24}])

disp('ttest Choice Control TOM (MA vs Ctrl)');
[h,p,~,stats]=ttest2([data{idx{2},24}],[data{idx{4},24}])%%%%%CHANGE NUMBERS

% mean consistency TOM for FTD
nanmean([data{idx{3},24}])%%%%%CHANGE NUMBERS
% mean consistency TOM for MA
nanmean([data{idx{2},24}])%%%%%CHANGE NUMBERS
% mean consistency TOM for CTRL
nanmean([data{idx{4},24}])%%%%%CHANGE NUMBERS


%% Plot Choice4DNR2AS results
for nvar = [26 27 30 33 36 39 42]
    figure('Color','w','Position',[0 0 300 300]);
    hold on;
    for ngroup = 2:length(idx)
        jitter = (rand(1,numel(idx{ngroup}))/10)-0.05;
        bar(ngroup,nanmean([data{idx{ngroup},nvar}]),'FaceColor',groupcol{ngroup});
        scatter(ngroup+jitter,[data{idx{ngroup},nvar}],30,'filled','MarkerFaceColor',[0.7 0.7 0.7],'MarkerEdgeColor','none');
        errorbar(ngroup,nanmean([data{idx{ngroup},nvar}]),nanstd([data{idx{ngroup},nvar}])/sqrt(numel(idx{ngroup})),'Color','k','LineStyle','none');
    end
    set(gca,'Xtick',2:length(idx),'XTicklabel',groupname);
    ylabel(varname(nvar));
    xtickangle(60);
...
[h,p,~,stats]=ttest2([data{idx{2},26}],[data{idx{4},26}])
...
[h,p,~,stats]=ttest2([data{idx{2},30}],[data{idx{4},30}])
...
[h,p,~,stats]=ttest2([data{idx{2},36}],[data{idx{4},36}])
...
[h,p,~,stats]=ttest2([data{idx{2},33}],[data{idx{4},33}])
...
[h,p,~,stats]=ttest2([data{idx{2},39}],[data{idx{4},39}])
...
[h,p,~,stats]=ttest2([data{idx{2},42}],[data{idx{4},42}])
...
[h,p,~,stats]=ttest([data{idx{3},28}],[data{idx{2},29}])
disp('ttest norm violation for high vs low reward level (MA)');
[h,p,~,stats]=ttest([data{idx{2},28}],[data{idx{3},29}])
...
[h,p,~,stats]=ttest([data{idx{3},32}],[data{idx{2},31}])
disp('ttest norm violation for high vs low norm level (MA)');
[h,p,~,stats]=ttest([data{idx{2},32}],[data{idx{3},31}])
...
excelData = xlsread('C:\Users\raphael.joly\ownCloud - JOLY Raphael (raphael.joly@icm-institute.org)@owncloud.icm-institute.org2\diagrammes résultats\statistiques\data_all.xlsx');

% Chargement de la cellule MATLAB contenant vos données
% load(data);

% Initialisation des indices des controles (1 à 8)
...
% Extraction des données de la colonne "disinhibition" pour les patients dans ControlIndices
disinhibitionData = excelData(rowPatientExcel, 11); % 11: colonne pour la DIFFERENCE de désinhibition dans le fichier excel

% Extraction des données de la colonne 26 de la cellule pour les patients dans ControlIndices
...
% Extraction des données de la colonne "disinhibition" pour les participants dans PatientIndices
disinhibitionDataPatient = excelData(rowPatientExcel, 12); % 12: colonne pour la désinhibition ACTUELLE dans le fichier excel
...
% Extraction des données de la colonne "disinhibition" pour les participants dans ControlIndices
disinhibitionDataControl = excelData(rowControlExcel, 12); % 12: colonne pour la désinhibition ACTUELLE dans le fichier excel
...
% Extraction des données de la colonne "disinhibition" pour les patients dans ControlIndices
disinhibitionData = excelData(rowPatientExcel, 11); % 11: colonne pour différence de désinhibition dans le fichier excel

% Extraction des données de la colonne 26 de la cellule pour les patients dans ControlIndices
cellData = cell2mat(data(rowPatientData, 30)); %30: sensibilité aux récompenses dans la tâche 3DNRS
