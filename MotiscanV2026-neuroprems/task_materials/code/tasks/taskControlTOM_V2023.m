function [sub_data] = taskControlTOM_V2023(sub_data,cfg)
% function [sub_data] = taskControlTOM_V2023(sub_data, cfg)
%
% taskControlTOM_V2023 - execute the Control_TOM choice task
% Launch the task with this function for testing one subject.
% The subject has to choose among two words the one that is closeest to the
% mental state of an individual that would observe the norm violation
%
%   task specifications:
...
%Updated March 2024 for V2024, A. CLEROUIN

%%liste choice
appariementTestingList1_file ='appariement_TOM_liste1.xlsx';
appariementTestingList2_file ='appariement_TOM_liste2.xlsx';
...
appariement_training = readtable('appariement_TOM_training.xlsx');
%appariement_testing = readtable('appariement_TOM.xlsx'); %now list1/2
...
for i=1:1
    instruction(i).texture = Screen('MakeTexture',display.window,imread(['instruction_TOM_'  num2str(i) '.bmp']));
end
...
text.question = 'D''après vous, un témoin de cette scène serait:'; % text.accept = 'Il pourrait m''arriver de';
...
isCorrectAnswerLeft = [zeros(1,6) ones(1,6)];
isCorrectAnswerLeft = isCorrectAnswerLeft(index);
...
% saving
if interrupt_task
    sub_data.(cfg.sessNber_str).tasks.taskControlTOM.completed...
        = false;
else; sub_data.(cfg.sessNber_str).tasks.taskControlTOM.completed...
        = true;
end
sub_data.(cfg.sessNber_str).tasks.taskControlTOM.results =      ...
