function [sub_data] = taskWeightR2E2_V2023_img_test(sub_data, cfg)
% function [sub_data] = taskWeightR2E2_V2(sub_data, cfg)
%
...
% training items
benefit_traininglist = readstim('rewards_training2.xlsx');
cost_traininglist    = readstim('efforts_training2.xlsx');
...
% Extract ratings
tempR   = sortrows(sub_data.(cfg.sessNber_str).tasks.taskRatingR2.results.data, 'itemNumber', 'ascend');
tempC   = sortrows(sub_data.(cfg.sessNber_str).tasks.taskRatingE2.results.data, 'itemNumber', 'ascend');

ratingsBenefit = tempR.rating(1:24);
ratingsCost    = tempC.rating(1:24);
rewardList     = readstim('rewards2.xlsx');
effortList     = readstim('efforts2.xlsx');
...
%% New fixed pairing
index = readtable('appariementR2E2.xlsx');
...
while iTrial<nTrial
    ...
    yshift = -100;
    DrawMyText(display.window,'J''accepte de',ftsz_small,[100 100 100],[x,y+1.5*yshift]);
    DrawMyText(display.window,effortList{stimE2(iTrial)},ftsz_small,[255 255 255],[x,y+1*yshift]);
    DrawMyText(display.window,'pour',ftsz_small,[100 100 100],[x,y-1*yshift]);
    DrawMyText(display.window,rewardList{stimR2(iTrial)},ftsz_small,[255 255 255],[x,y-1.5*yshift]);
    % Display responses
    xshift = x/2;
...
data          = table((1:nTrial)',stimR2,ratingBenefit,stimE2,ratingCost,isLeftChoice',choice_position',choicedo',rt','VariableNames',varNames);

% saving
sub_data.(cfg.sessNber_str).tasks.taskWeightRE.results = struct('trainingData', training_data, 'data', data);
end
