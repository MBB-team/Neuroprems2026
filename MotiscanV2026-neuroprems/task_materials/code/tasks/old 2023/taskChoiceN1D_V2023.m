function [sub_data] = taskChoiceN1D_V2023(sub_data,cfg)
% function [sub_data] = taskChoiceN1D_V2023(sub_data, cfg, dimension)
%
...
%-----------------------------------------------
taskName = ['ChoiceN'];
taskChoice_strctName = ['taskChoiceN'];
...
appariement = readtable(pairingList_file)
instruc_fill      = 'VIOLATIONS DE NORMES présentées';
imgItem = struct('texture',{},'position',{});
for i=1:24
imgItem(i).texture = Screen('MakeTexture',display.window,imread(['img_SocialNormViolations_' num2str(i) '.bmp']));
end
imgTraining = struct('texture',{},'position',{});
for i=1:6
imgTraining(i).texture = Screen('MakeTexture',display.window,imread(['img_SocialNormViolations_training_' num2str(i) '.bmp']));
end
appariementTrainingList_file = 'appariementN.xlsx'; 
...
% add images per dimension
img.N = imgTraining(training_options).texture;
...
while iTrial<nTrial
    ...
    img.l = testingList{list(1,iTrial)};
    img.r = testingList{list(2,iTrial)};
...
function [] = disp_trial(display,ftsz,max_charPline,text,img,~,~,   ...
    ~,~,show_response,varargin)
...
DrawFormattedText(display.window,text.pour,'center','center',[100 100 100],max_charPline,0,0,2,0);
DrawFormattedText(display.window,testingList_file,'center',9*y/8,[255 255 255],max_charPline,0,0,2,0);
Screen('DrawTexture',display.window,img.R,[],rectImg2);

DrawFormattedText(display.window,text.pour,'center','center',[100 100 100],max_charPline,0,0,2,0);
DrawFormattedText(display.window,text.R,'center',9*y/8,[255 255 255],max_charPline,0,0,2,0);
Screen('DrawTexture',display.window,img.R,[],rectImg2);
