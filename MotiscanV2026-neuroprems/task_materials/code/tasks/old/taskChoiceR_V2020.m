function [sub_data] = taskChoiceR_V2020(sub_data, cfg)
% function [sub_data] = taskChoiceR_V2(sub_data, cfg)
%
% taskChoiceR - execute the reward 1-D choice task (from motiscan battery V2)
% Launch the task with this function for testing one subject.
% The subject has to choose between 2 items his prefered one.
%
%   task specifications:
%       - structure: instructions -> training -> testing
%       - experimental conditions: item value difference, item subdomain
%       (alimentary, non-alimentary and social rewards)
%       - randomization:
...
% Extract ratings
temp    = sortrows(sub_data.(cfg.sessNber_str).tasks.taskRatingR.results.data, 'itemNumber', 'ascend');
...
%% NEW FIXED PAIRING
list = readtable('appariementR.xlsx');
...
while iTrial<nTraining
    ...
    DrawMyText(display.window,trainingList{training_options(1,iTrial)},ftsz_small,[255 255 255],[1/2*x,y]);
    DrawMyText(display.window,trainingList{training_options(2,iTrial)},ftsz_small,[255 255 255],[3/2*x,y]);
        ...
        if keyisdown==1
            ...
            if keycode(key.escape)==1
                ...
                elseif keycode(key.right)==1
                    exit=1;
                    timedown=GetSecs;
                    training_isLeftChoice(iTrial)=0;                                               %0=right
                    repeat=0;
                end
            end
        end
    ...
    if repeat==0 
        ...
        DrawMyText(display.window,trainingList{training_options(1,iTrial)},ftsz_small,left_color,[1/2*x,y]);
        right_color = [255*(training_isLeftChoice(iTrial)==1) 255 255*(training_isLeftChoice(iTrial)==1)];
        DrawMyText(display.window,trainingList{training_options(2,iTrial)},ftsz_small,right_color,[3/2*x,y]);
        Screen('LineStipple',display.window, 1, 2, mod(ceil((1:16)/8),2));
        Screen('DrawLine', display.window, [255 255 255]*0.5, x, y*1/2 ,  x, y*3/2, 3);
        Screen(display.window,'Flip');
        tresponse = GetSecs;
...
while iTrial<nTrial
    ...
    % Write instructions & options
    DrawMyText(display.window,'Que préférez-vous ?',ftsz_small,[100 100 100],[x,y-300]);
    DrawMyText(display.window,rewardList{list(1,iTrial)},ftsz_small,[255 255 255],[1/2*x,y]);
    DrawMyText(display.window,rewardList{list(2,iTrial)},ftsz_small,[255 255 255],[3/2*x,y]);
    Screen('LineStipple',display.window, 1, 2, mod(ceil((1:16)/8),2));
    Screen('DrawLine', display.window, [255 255 255]*0.5, x, y*1/2 ,  x, y*3/2, 3);
    startime = Screen(display.window,'Flip');
    ...
    if repeat==0
        ...
        DrawMyText(display.window,rewardList{list(1,iTrial)},ftsz_small,left_color,[1/2*x,y]);
        right_color = [255*(isLeftChoice(iTrial)==1) 255 255*(isLeftChoice(iTrial)==1)];
        DrawMyText(display.window,rewardList{list(2,iTrial)},ftsz_small,right_color,[3/2*x,y]);
        Screen('LineStipple',display.window, 1, 2, mod(ceil((1:16)/8),2));
        Screen('DrawLine', display.window, [255 255 255]*0.5, x, y*1/2 ,  x, y*3/2, 3);
        Screen(display.window,'Flip');
        tresponse = GetSecs;
...
% saving
sub_data.(cfg.sessNber_str).tasks.taskChoiceR.results = struct('trainingData', training_data, 'data', data);
