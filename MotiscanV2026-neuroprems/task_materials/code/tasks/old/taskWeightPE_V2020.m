function [sub_data] = taskWeightPE_V2020(sub_data, cfg)
%function [sub_data] = taskWeightRP_V2(sub_data,cfg)
%
% taskWeightPE - execute the punishment/effort 2-D choice task (from motiscan battery)
...
% training items
benefit_traininglist = readstim('punishments_training.xlsx');
cost_traininglist    = readstim('efforts_training.xlsx');
...
% Extract ratings
tempR   = sortrows(sub_data.(cfg.sessNber_str).tasks.taskRatingP.results.data, 'itemNumber', 'ascend');
tempC   = sortrows(sub_data.(cfg.sessNber_str).tasks.taskRatingE.results.data, 'itemNumber', 'ascend');
...
% 
% stimP = stimB(randomtrial);
% stimE = stimC(randomtrial);

%% New fixed pairing
index = readtable('appariementPE.xlsx');
...
while iTrial<nTraining
    ...
    DrawMyText(display.window,'pour éviter de',ftsz_small,[100 100 100],[x,y-1*yshift]);
...
while iTrial<nTrial
    ...
    yshift = -100;
    DrawMyText(display.window,'J''accepte de',ftsz_small,[100 100 100],[x,y+1.5*yshift]);
    DrawMyText(display.window,effortList{stimE(iTrial)},ftsz_small,[255 255 255],[x,y+1*yshift]);
    DrawMyText(display.window,'pour éviter de',ftsz_small,[100 100 100],[x,y-1*yshift]);
    DrawMyText(display.window,punishList{stimP(iTrial)},ftsz_small,[255 255 255],[x,y-1.5*yshift]);
    % Display responses
    xshift = x/2;
...
ratingBenefit = ratingsBenefit(stimP(:));
ratingCost    = ratingsCost(stimE(:));
data          = table((1:nTrial)',stimP,ratingBenefit,stimE,ratingCost,isLeftChoice',choice_position',choicedo',rt','VariableNames',varNames);

% saving
sub_data.(cfg.sessNber_str).tasks.taskWeightPE.results = struct('trainingData', training_data, 'data', data);
