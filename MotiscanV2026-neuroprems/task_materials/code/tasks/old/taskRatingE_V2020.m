function [sub_data] = taskRatingE_V2020(sub_data, cfg)
...
while ntrial<nTrial
    ...
    while exit==0
        % Write reward
        DrawMyText(display.window,effortList{trialperm(ntrial)},ftsz_small,[255 255 255],[x,y]);
...
% saving
sub_data.(cfg.sessNber_str).tasks.taskRatingE.results = struct('trainingData', training_data, 'data', data);
% Do we really need to save this ?
sub_data.(cfg.sessNber_str).tasks.taskRatingE.trainingList = trainingList;
sub_data.(cfg.sessNber_str).tasks.taskRatingE.effortList   = effortList;
