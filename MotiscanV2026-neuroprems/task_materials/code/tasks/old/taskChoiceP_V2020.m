function [sub_data] = taskChoiceP_V2020(sub_data, cfg)
% function [sub_data] = taskChoiceP_V2(sub_data, cfg)
%
% taskChoiceP - execute the effort 1-D choice task (from motiscan battery V2)
...
%       (sensory, non-sensory and social punitions)
%       - randomization:
...
trainingList = readstim('punishments_training.xlsx');
...
% Extract ratings
temp    = sortrows(sub_data.(cfg.sessNber_str).tasks.taskRatingP.results.data, 'itemNumber', 'ascend');
...
% 
%% NEW FIXED PAIRING
list = readtable('appariementP.xlsx');
...
while iTrial<nTrial 
    ...
    % Write instructions & options
    DrawMyText(display.window,'Que préférez-vous ?',ftsz_small,[100 100 100],[x,y-300]);
    DrawMyText(display.window,punishList{list(1,iTrial)},ftsz_small,[255 255 255],[1/2*x,y]);
    DrawMyText(display.window,punishList{list(2,iTrial)},ftsz_small,[255 255 255],[3/2*x,y]);
    Screen('LineStipple',display.window, 1, 2, mod(ceil((1:16)/8),2));
    Screen('DrawLine', display.window, [255 255 255]*0.5, x, y*1/2 ,  x, y*3/2, 3);
    startime = Screen(display.window,'Flip');
    ...
    if repeat==0
        ...
        DrawMyText(display.window,punishList{list(1,iTrial)},ftsz_small,left_color,[1/2*x,y]);
        right_color = [255*(isLeftChoice(iTrial)==1) 255 255*(isLeftChoice(iTrial)==1)];
        DrawMyText(display.window,punishList{list(2,iTrial)},ftsz_small,right_color,[3/2*x,y]);
        Screen('LineStipple',display.window, 1, 2, mod(ceil((1:16)/8),2));
        Screen('DrawLine', display.window, [255 255 255]*0.5, x, y*1/2 ,  x, y*3/2, 3);
        Screen(display.window,'Flip');
        tresponse = GetSecs;
...
%-----------------------------------------------
% data creation
varNames      = {'trialNumber', 'choice', 'isLeftChoice', 'RT'};
...
% saving
sub_data.(cfg.sessNber_str).tasks.taskChoiceP.results = struct('trainingData', training_data, 'data', data);
