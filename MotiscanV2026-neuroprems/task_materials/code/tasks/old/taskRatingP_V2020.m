function [sub_data] = taskRatingP_V2020(sub_data, cfg)
% function [sub_data] = taskRatingP_V2(sub_data, cfg)
%
% taskRatingP - execute the punishment rating task (from motiscan battery)
...
%       - experimental conditions:  item subdomain (sensory,
%       non-sensory, social and money punitions)
...
emoji_infinite=Screen('MakeTexture',display.window,imread('emoji_punishment.png'));
...
moneyList = {'Perdre 1 pièce de 1 centime','Perdre 1 pièce de 1 euro','Perdre 1 billet de 20 euros'};
...
nTrial = nTrial + 3;
punishList = [ punishList , moneyList];
...
% Ratings transformation (to 0:1:10)
...
while ntrial<nTrial
    ...
    while exit==0
        % Write reward
        DrawMyText(display.window,punishList{trialperm(ntrial)},ftsz_small,[255 255 255],[x,y]);
...
% saving
sub_data.(cfg.sessNber_str).tasks.taskRatingP.results = struct('trainingData', training_data, 'data', data);
% Do we really need to save this ?
sub_data.(cfg.sessNber_str).tasks.taskRatingP.trainingList = trainingList;
sub_data.(cfg.sessNber_str).tasks.taskRatingP.punishList   = punishList;
