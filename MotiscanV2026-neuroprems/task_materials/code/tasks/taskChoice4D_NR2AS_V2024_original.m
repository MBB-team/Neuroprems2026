...
stim2_duration = 3; % second option(s) on scree
...
ratingsBenefit_training  = tempB_training.rating(1:6);
...
%stimOrder = [repmat(1,1,24) repmat(2,1,24)];
%stimN = [1:24 1:24];

%stimOrder = [repmat(1,1,12) repmat(2,1,12) repmat(3,1,12) repmat(4,1,12)];
%%plus besoin de stimOrder si on retire la manipulation attentionnelle

stimN = [1:12 1:12 1:12 1:12]; %chaque norme repete 4 fois

% stimR = repmat([repmat(1,1,8) repmat(2,1,8) repmat(3,1,8)],1,2); %% Error (Rlevel not orthogonalized with stimN)
stimR = [repmat(1,1,8) repmat(2,1,8) repmat(3,1,8)]; 
...
%stimS = repmat([repmat(0,1,4) repmat(1,1,4)],1,3);
%stimS = [stimS,-1.*stimS+1]; % Balancing StimS for the two repetitions of norms

%stimAS = repmat([repmat(0,1,4) repmat(1,1,4)],1,3);
%stimAS = [stimAS,-1.*stimAS+1]; % Balancing StimAS for the two repetitions of norms

stimS = [repmat(0,1,12) repmat(1,1,12)];
stimS = [stimS,-1.*stimS+1];

stimAS = repmat([repmat(0,1,12) repmat(1,1,12)],1,2);


%stimS = [repmat([0 1], 1, 6), repmat([1 0], 1, 6), repmat([0 1], 1, 6), repmat([0 1], 1, 6)];
%stimAS = [repmat([0 1], 1, 6), repmat([1 0], 1, 6), repmat([0 1], 1, 6), repmat([0 1], 1, 6)];


index = randperm(48);
%stimOrder = stimOrder(index);% 1=reward first, 2=Costs first 
%%getting rid of stimOrder
stimN = stimN(index);
stimR = stimR(index);
stimS = stimS(index);
stimAS = stimAS(index);
...
training_options = [1 2 3 4 5 6 ; % norm number
                    ...
                    0 1 1 0 0 1]; %AS level
%add aversive sanction level - voir fonction nombre essais

%training_stimOrder = [1 2 1 2 1 2]; %same reason no attentional
...
training_data = table((1:nTraining)',...
                      ...
                      training_options(2,:)',ratingsBenefit_training(training_options(2,:)),...
...
ratingAversiveSanction = ratingsAversiveSanction(stimS_Nbr).*stimAS';
