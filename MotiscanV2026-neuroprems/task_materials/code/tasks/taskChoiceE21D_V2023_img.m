function [sub_data] = taskChoiceE21D_V2023_img(sub_data,cfg)
% function [sub_data] = taskChoiceE21D_V2023_img(sub_data, cfg)
%
...
%-----------------------------------------------
taskName = 'ChoiceE21D'
taskChoice_strctName = 'taskChoiceE21D'
taskRating_strctName = 'taskRatingE2'

trainingList_file = 'efforts_training2.xlsx';
testingList_file  = 'efforts2.xlsx';
pairingList_file  = 'appariementE2.xlsx';
appariement_testing = readtable(pairingList_file)
instruc_fill      = 'EFFORTS présentés';
...
text.instrucText = sprintf(['Dans ce test, il vous est demandé de '     ...
    'choisir entre deux %s sous forme de texte et d''images.\n\n'                      ...
...
while iTrial < nTraining
    ...
    img.l = imgTrainingE2(training_options(1,iTrial)).texture;
    % DrawMyText(display.window,trainingList{training_options(1,iTrial)},ftsz_small,[255 255 255],[1/2*x,y],max_charPline);
    DrawFormattedText(display.window,text.l,'center',13*y/8,          ...
        [255 255 255],max_charPline, 0, 0, 1, 0, winL_coor);
    text.r = trainingList{training_options(2,iTrial)};
    img.r = imgTrainingE2(training_options(2,iTrial)).texture;
    % DrawMyText(display.window,trainingList{training_options(2,iTrial)},ftsz_small,[255 255 255],[3/2*x,y],max_charPline);
    DrawFormattedText(display.window,text.r,'center',13*y/8,          ...
        [255 255 255],max_charPline, 0, 0, 1, 0, winR_coor);
    ...
    while exit == 0
        ...
        Screen('TextSize', display.window, ftsz_small);
        % DrawMyText(display.window,trainingList{training_options(1,iTrial)},ftsz_small,[255 255 255],[1/2*x,y],max_charPline);
        DrawFormattedText(display.window,text.l,'center',13*y/8,[255 255 255],max_charPline, 0, 0, 1, 0, winL_coor);
        % DrawMyText(display.window,trainingList{training_options(2,iTrial)},ftsz_small,[255 255 255],[3/2*x,y],max_charPline);
        DrawFormattedText(display.window,text.r,'center',13*y/8,[255 255 255],max_charPline, 0, 0, 1, 0, winR_coor);
    ...
    if repeat==0
        ...
        DrawFormattedText(display.window,text.l,'center',13*y/8,left_color,max_charPline, 0, 0, 1, 0, winL_coor);
        ...
        DrawFormattedText(display.window,text.r,'center',13*y/8,right_color,max_charPline, 0, 0, 1, 0, winR_coor);
...
while iTrial<nTrial
    ...
    img.l = imgItemE2(list(1,iTrial)).texture;
    img.r = imgItemE2(list(2,iTrial)).texture;
