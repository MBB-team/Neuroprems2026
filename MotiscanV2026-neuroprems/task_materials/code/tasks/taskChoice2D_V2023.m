function [sub_data] = taskChoice2D_V2023(sub_data,cfg)
% function [sub_data] = taskChoice2D_V2023(sub_data, cfg)
%
...
% Updated May 2023 for V2023, R.JOLY
...
beneficeTestingList_file = 'rewards2.xlsx';
costTestingList_file     = 'efforts2.xlsx';

pairingList_file  = 'appariementR2E2.xlsx';

instruc_fill = {'d''un EFFORT à effectuer','obtenir une RECOMPENSE',...
    'la récompense vaut-elle la peine d''effectuer l''effort'};
        %'la récompense','d''effectuer l''effort'};
...
%         instruc_fill = {'d''une PUNITION à subir','obtenir une RECOMPENSE',...
%             'la récompense vaut-elle le désagrément de subir la punition'};
%         %'la récompense','de subir la punition'};
%     case 'Ed'
...
%         
%         instruc_fill = {'d''un EFFORT à effectuer','éviter une PUNITION',...
%             'l''effort vaut-il la peine d''être effectué pour éviter la punition'}; 
%         %s vaut-elle la peine %s', 'la punition à éviter','d''effectuer l''effort'};
% end
...
text.instrucText1 = sprintf(['Dans ce test, il vous est demandé '       ...
    'd''accepter ou refuser différentes offres.\n\nChaque offre est '   ...
    'composée %s pour %s.\n Autrement dit, %s ?'],                      ...
    instruc_fill{:});
text.instrucText2 = ['Touchez du côté correspondant à votre réponse '   ...
...
tempB = sortrows(sub_data.(cfg.sessNber_str).tasks.(benefit_ratingTaskName).results.data,...
    'itemNumber', 'ascend');
tempC = sortrows(sub_data.(cfg.sessNber_str).tasks.(cost_ratingTaskName).results.data,...
    'itemNumber', 'ascend');
