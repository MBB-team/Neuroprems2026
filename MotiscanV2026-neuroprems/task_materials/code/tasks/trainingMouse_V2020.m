function [sub_data] = trainingMouse_V2020(sub_data, cfg)
% function [sub_data] = trainingMouse_V2(sub_data, cfg)
%
% Training task, the subject has to click on various illuminated targets in
...
textstring = ['Au cours de ce test, vous allez vous familiariser avec l''utilisation de l''interface. ',...
    'Une échelle graduée et une cible seront affichées à l’écran. Votre but est de cliquer sur la cible. ',...
    'Prenez le temps qu''il vous faut pour effectuer un pointage précis. '];
...
while ntrial<nTraining
    ...
    while exit==0
        ...
        [xMouse,~,buttons] = GetMouse(display.window);
    ...
    display_rating_likert(display.window,x,y,rating(ntrial),target(ntrial))
    Screen(display.window,'Flip');
    WaitSecs(blanktime);
end
...
varNames = {'trialNumber', 'position', 'target', 'press_RT', 'validation_RT'};
...
sub_data.(cfg.sessNber_str).tasks.trainingResp.results = struct('data',data);
