function [sub_data] = taskRatingS_V2023_img(sub_data, cfg)
% function [sub_data] = taskRatingS_V2023_img(sub_data, cfg)
%
% taskRatingN - execute the social norm rating task (from motiscan battery V2)
% Launch the task with this function for testing one subject.
% The subject has to provide a subjective estimate of item social norm violation.
%
%   task specifications:
...
% emoji_infinite=Screen('MakeTexture',display.window,imread('emoji_happy.png'));
rect_emoji_infinite=CenterRectOnPoint(Screen('Rect',emoji_infinite),xScaleLim(2),yemoji);

% instructions images
instruction = struct('texture',{},'position',{});
for i=1:3
    instruction(i).texture = Screen('MakeTexture',display.window,imread(['instruction_R' taskName(2:end) '_' num2str(i) '.bmp']));
end

cd(cfg.paths.task.text) % extracting text

% text items
itemList = readstim('SocialSanctions.xlsx')';
itemList = itemList(:,2); %keep only sanctions_level_1
trainingList = readstim('SocialSanctions_training.xlsx')';
trainingList = trainingList(:,2); %keep only sanctions_level_1
...
for i=1:12
    imgItem(i).texture = Screen('MakeTexture',display.window,imread(['img_SocialSanctions1_' num2str(i) '.bmp']));
end
imgTraining = struct('texture',{},'position',{});
for i=1:6
    imgTraining(i).texture = Screen('MakeTexture',display.window,imread(['img_SocialSanctions1_training_' num2str(i) '.bmp']));
end
...
while ntrial<nTraining
    ...
    while exit==0
        ...
        % Write instructions
        question_name = 'Ce que cette personne pense de moi m''importe';
...
% saving
sub_data.(cfg.sessNber_str).tasks.taskRatingS.results = struct('trainingData', training_data, 'data', data);
% Do we really need to save this ?
sub_data.(cfg.sessNber_str).tasks.taskRatingS.trainingList = trainingList;
sub_data.(cfg.sessNber_str).tasks.taskRatingS.itemList = itemList;
